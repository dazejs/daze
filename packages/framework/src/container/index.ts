...
export class Container extends EventEmitter {
  ...
  private invokeInjectAbleClass(abstract: any, args: any[]) {
    ...
    return new Proxy(instance, {
      get(target, property, receiver) {
        if (property === 'name' || property === 'constructor' || typeof property !== 'string') return Reflect.get(target, property, receiver);
  ...
  private getMethodBindParams(Concrete: any, key: string, args: any[] = []) { 
    const injectParams: any[] = [];
    const typeParams: any[] = [];
    const methodInjectors = Reflect.getMetadata(
      'injectparams', Concrete, key,
    ) || [];
    const methodParamtypes = Reflect.getMetadata(symbols.PARAMTYPES_METADATA, Concrete, key) ?? [];
    for (const [type, params = [], handler] of methodInjectors) {
      let injectedParam = this.make(type, [...params, ...args]);
      if (typeof handler === 'function') {
        injectedParam = handler(injectedParam);
      }
      injectParams.push(injectedParam);
    }
    for (const paramtype of methodParamtypes) {
      const injectedParam = this.make(paramtype, [...args]);
      typeParams.push(injectedParam);
    }
    return [...injectParams.reverse(), ...typeParams];
  }

  private getPropertyParam(Concrete: any, key: string, args: any[] = []) { 
